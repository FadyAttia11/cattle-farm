  ...
  <main id="main">
    ...
    <section id="portfolio-details" class="portfolio-details">
      <div class="container">
        ...
        <div class="portfolio-description">
          <h2>Contact Farm To make Reservations</h2>
          <p>(Ask The Farm Anything Related To this Product)</p>
            <form method="post" enctype="multipart/form-data" class="needs-validation" novalidate>

                <input type="number" class="form-control mb-3" placeholder="Quantity" name="quantity" required>
                <textarea class="form-control mb-3" rows="5" name="message" placeholder="Your Message.."></textarea>

                <button type="submit" class="btn btn-primary">Ask About This Product</button>
                <?php
                    if($_SERVER['REQUEST_METHOD'] == "POST") {
                        if(isset($_POST['quantity'])) {
                            $farm_name = $product_data['farm_name'];
                            $product_name = $product_data['product_name'];
                            $quantity = $_POST['quantity'];
                            $message = $_POST['message'];
    
                            $query = "insert into products_reserv (customer_name,farm_name,product_name,quantity,message,response) values ('$customer_name','$farm_name','$product_name','$quantity','$message','')";
                            $result = mysqli_query($con, $query);
    
                            if($result) {
                                echo "Successfully sent your message to the farm";
                            } else {
                                echo "error sending your message to the farm";
                            }
                        }
                    }
                ?>
